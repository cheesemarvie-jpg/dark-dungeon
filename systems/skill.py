...
class SkillManager:
    ...
    def create_skill_pool(self):
        return [
            ...
            Skill(
                "Light Boost",
                "Player light radius increases",
                lambda status: setattr(status, "plight_level", status.plight_level + 50)
            ),
            Skill(
                "Goal Vision",
                "Goal light radius increases",
                lambda status: setattr(status, "glight_level", status.glight_level + 100)
            ),
            Skill(
                "Moratorium Up",
                "Darkness delay increases",
            ...
            Skill(
                "Lucky Flash",
                "Occasionally illuminates the whole maze.",
