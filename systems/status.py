class Status:#ゲーム中強化されていくステータスを保持する
    def __init__(self):
        ...
        self.wisp_count = 0
        self.flash_stack = 0
        self.new_skill = 0#新たなスキルを取得可能か判定
        self.exp = 0
    ...
    def set_exp(self, exp):
        while exp > 0:
            if self.exp + exp < self.get_exp_require():
                self.exp += exp
                exp = 0
            else:
                exp -= self.get_exp_require() - self.exp
                self.exp = 0
                self.new_skill += 1
    ...
    def get_exp_require(self):
        return  self.get_timelimit() * 2