...
class Result(Scene):
    ...
    def __init__(self, parent, status):
        ...
        if status.get_stagecount() > 15:
            self.win_flag = True
            self.group.add(
                TextSprite("You Won!", font, (128,128,128),
            ...
            # ステージ表示
            self.group.add(
            TextSprite("You Passed All Floors.", font, (128,128,128),
                       (center_x, center_y + 120))
            )
            
            self.group.add(
                TextSprite(f"Clear Time: {status.time/1000:.2f}s",
                           font, (128,128,128),
            ...
            self.win_flag = False
            self.group.add(
                TextSprite("You Lose...", font, (128,128,128),
                           (center_x, center_y + 50))
            )
            # ステージ表示
            self.group.add(
            TextSprite(f"You Passed {status.get_stagecount() - 1} Floors.", font, (128,128,128),
                       (center_x, center_y + 120))
            )
